    ...
    public partial class Form1 : Form
    {
        ...
        private void ButtonGenerateLicense_Click(object? sender, EventArgs e)
        {
            try
            {
                ...
                // 生成许可证文件
                byte[] licenseBytes = _encryptionService.GenerateEncryptedLicense(licenseInfo);
                ...
                using (SaveFileDialog saveFileDialog = new SaveFileDialog())
                {
                    ...
                    {
                        File.WriteAllBytes(saveFileDialog.FileName, licenseBytes);
        ...
        private void ButtonValidateLicense_Click(object? sender, EventArgs e)
        {
            using (OpenFileDialog openFileDialog = new OpenFileDialog())
            {
                ...
                if (openFileDialog.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        byte[] licenseBytes = File.ReadAllBytes(openFileDialog.FileName);
                        var licenseInfo = _encryptionService.DecryptAndValidateLicense(licenseBytes);

                        if (licenseInfo != null)
                        {
                            string message = $"License Validation Result:\n\n" +
                                           $"Valid: Yes\n" +
                                           $"License Count: {licenseInfo.LicenseCount}\n" +
                                           $"Expiration Date: {licenseInfo.ExpirationDate:yyyy-MM-dd HH:mm:ss}\n" +
                                           $"Customer Name: {licenseInfo.CustomerName}\n" +
                                           $"Notes: {licenseInfo.Notes}\n" +
                                           $"MAC Addresses: {string.Join(", ", licenseInfo.MacAddresses)}\n" +
                                           $"Restrict Application: {(licenseInfo.RestrictApplication ? "Yes" : "No")}\n" +
                                           $"Applications: {string.Join(", ", licenseInfo.Applications.Select(a => a.Name))}\n\n";

                            MessageBox.Show(message, "License Validation", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        else
                        {
                            MessageBox.Show("License validation failed - invalid or corrupted license file.", "License Validation", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        private void ButtonEncryptDll_Click(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SaveFileDialog saveFileDialog = new SaveFileDialog())
                {
                    saveFileDialog.Filter = "MARS Binary Files (*.marsbin)|*.marsbin|All Files (*.*)|*.*";
                    saveFileDialog.Title = "Save Encrypted DLL File";
                    saveFileDialog.FileName = Path.GetFileNameWithoutExtension(textBoxDllFilePath.Text) + ".marsbin";

                    if (saveFileDialog.ShowDialog() == DialogResult.OK)
                    {
                        // 加密DLL文件（使用固定AES密钥）
                        bool success = _dllEncryptionService.EncryptDllFile(textBoxDllFilePath.Text, saveFileDialog.FileName);

                        if (success)
                        {
                            MessageBox.Show($"DLL file encrypted successfully!\nOutput: {saveFileDialog.FileName}\n\nEncryption method: AES with key 'MARsAutomationTest'\nPadding: 888 random chars + encrypted data + 520 random chars", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        else
                        {
        ...
        private void ButtonSelectEncryptedFile_Click(object? sender, EventArgs e)
        {
            using (OpenFileDialog openFileDialog = new OpenFileDialog())
            {
                openFileDialog.Filter = "MARS Binary Files (*.marsbin)|*.marsbin|All Files (*.*)|*.*";
        ...
        private void ButtonDecryptDll_Click(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SaveFileDialog saveFileDialog = new SaveFileDialog())
                {
                    ...
                    {
                        // 解密DLL文件（使用固定AES密钥）
                        bool success = _dllEncryptionService.DecryptDllFile(textBoxEncryptedFilePath.Text, saveFileDialog.FileName);

                        if (success)
                        {
                            MessageBox.Show($"DLL file decrypted successfully!\nOutput: {saveFileDialog.FileName}\n\nDecryption method: AES with key 'MARsAutomationTest'", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        else
                        {
        ...
        private void ButtonDecryptToStream_Click(object? sender, EventArgs e)
        {
            ...
            {
                // 解密到流（使用固定AES密钥）
                byte[] decryptedDllBytes = _dllEncryptionService.DecryptDllToStream(textBoxEncryptedFilePath.Text);

                // 显示解密结果信息
                string message = $"DLL decrypted to stream successfully!\n" +
                               $"Decrypted DLL size: {decryptedDllBytes.Length:N0} bytes\n" +
                               $"Decryption method: AES with key 'MARsAutomationTest'\n" +
