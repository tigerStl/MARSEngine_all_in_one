...
namespace MarsTestFrame.SourceCode.com.Mars.BusinessLogic
{
    public class MarsTestFrameBusinessLogic
    {
        ...
        public static IList<MarsKeyValues<string, string>> GetAvailableAppsByProjectIdAndStoryBoardID(string strDBIdx, long projectId, long storyboardId, ref ERROR_CODE eCde, ref List<MarsKeyValues<string, string>> lstUnavailableApps)
        {
            Logger.Info("GetAvailableAppsByProjectIdAndStoryBoardID",string.Format("ProjectId:[{0}] storyBoardId:[{1}]", projectId, storyboardId));
            List<MarsKeyValues<string, string>> lstResult = DashBoardFactory.RefreshProjectsWithApps(strDBIdx, projectId, storyboardId);
