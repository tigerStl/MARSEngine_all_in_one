...
namespace Route2NSEx.src.Marquis.systemUtil
{
    public class MLogger
    {
        ...
        public void logBegin(string strMethodName, string paraInfo, [CallerLineNumber] int ln = 0)
        {
#if !_tigerHintLog
            //mobjLog.Info(string.Format("[INFO] {0:MM/dd/yyyy H:mm:ss zzz} {1}.{2} begins...", DateTime.Now, mstrCurrentClassName, strMethodName));
#endif
            mobjLog.Info(string.Format("[INFO] #{3}|{0}.{1} begins...Paras:{2}", mstrCurrentClassName, strMethodName, paraInfo, ln));

        }
        public void logBegin(string strMethodName, [CallerLineNumber] int ln=0)
        {
#if !_tigerHintLog
            //mobjLog.Info(string.Format("[INFO] {0:MM/dd/yyyy H:mm:ss zzz} {1}.{2} begins...", DateTime.Now, mstrCurrentClassName, strMethodName));
#endif
            mobjLog.Info(string.Format("[BEGIN] [#{2}] {0}.{1} begins...", mstrCurrentClassName, strMethodName, ln));
        ...
        {
            mobjLog.Info($"[INFO] LN:#[{iLn}],[{strMethodName}]-[{DateTime.Now:yyyy-MM-dd:HH:mm:ss fff}] {strInfo}");
                //string.Format("[INFO] LN:#[{4}],MemberName:[{5}]- {1}.{2} {3}", DateTime.Now, mstrCurrentClassName, strMethodName, strInfo, iLn, strName));
        }
...
#endif
        public void Debug(string strMethodName, string debugInfo)
        {
            mobjLog.Debug($"-----------{strMethodName}---------{debugInfo}");
        }
        ...
        {
            mobjLog.Error(string.Format("[ERROR] LN:#[{4}] {1}.{2} {3}", DateTime.Now,
        ...
        {
            mobjLog.Error(string.Format("[ERROR] LN:#[{4}] {1}.{2} {3}, trace:[5]", DateTime.Now,
