...
namespace Mars.message.Hooks.Utilities {
    public enum VKeys
    {
    ...
    public static class SendKeysConverter
    {
        private static readonly Dictionary<int, string> VKeyToSendKeysMap = new Dictionary<int, string>()
        {
            { 0x08, "{BACKSPACE}" },
            { 0x09, "{TAB}" },
            { 0x0D, "{ENTER}" },
        ...
        public class MarsKeyPressStatus
        {
            public VKeys key { get; set; }
            public bool isContorlPress { get; set; } = false;
            public bool isShiftPress { get; set; } = false;

        }

        public static string ConvertVKeysToSendKeys(List<VKeys> vKeys)
        {
            var result = new StringBuilder();

            foreach (var vKey in vKeys)
            {
                // Handle alphabetic and numeric keys
                if (((int)vKey >= 0x30 && (int)vKey <= 0x39) || ((int)vKey >= 0x41 && (int)vKey <= 0x5A))
                {
                    result.Append((char)vKey); // Add the character directly
                }
                // Handle special keys
                else if (VKeyToSendKeysMap.ContainsKey((int)vKey))
                {
                    result.Append(VKeyToSendKeysMap[(int)vKey]);
                }
                else
                {
                    Console.WriteLine($"Warnning: No mapping found for VKey: {vKey}");
                }
            }
        ...
        public static string ConvertVKeysToSendKeys(List<MarsKeyPressStatus> vKeys, bool isShiftPressed)
        {
            var result = new StringBuilder();
            string strKey = "";
            foreach (var vKey in vKeys)
            {
                // Handle alphabetic and numeric keys
                if (((int)vKey.key >= 0x30 && (int)vKey.key <= 0x39) || ((int)vKey.key >= 0x41 && (int)vKey.key <= 0x5A))
                {
                    strKey = (char)(vKey.key)+"";
                    if (vKey.isShiftPress) {
                        strKey = $"+{strKey}";
                    }
                    if (vKey.isContorlPress)
                    {
                        strKey = $"^{strKey}";
                    }
                    if (!isShiftPressed) {
                    ...
                    result.Append(strKey); // Add the character directly
                }
                // Handle special keys
                else if (VKeyToSendKeysMap.ContainsKey(((int)vKey.key)))
                {
                    strKey = VKeyToSendKeysMap[((int)vKey.key)];
                    if (vKey.isShiftPress)
                    {
                        strKey = $"+{strKey}";
                    }
    ...
    /// </summary>
    public class MarsGlobalKeyboardHook {
		private static MarsGlobalKeyboardHook instance = null; 

		public static MarsGlobalKeyboardHook startKeyboardHook()
		{
			if (instance==null)
			    instance = new MarsGlobalKeyboardHook();
            return instance;
		}
        ...
        public List<VKeys> current_key_buffer = new List<VKeys>();
        ...
        {            
            if (nCode >= 0)
            {
                ...
                MarsLoggerSimple.DEBUG("HookFunc", $"keydown|wparam|{wParam}|{lParam}");
                switch (iwParam)
                {
                    case WM_KEYDOWN:
                    case WM_SYSKEYDOWN:                        
                    ...
                    case WM_KEYUP:
                        ...
                        break;
                    case WM_CHAR:
                        KeyPress?.Invoke((VKeys)Marshal.ReadInt32(lParam));
                        break;
                }                
            }
