...
namespace Mars.Business
{
    public class B_V_TEST_DATA_REPORT_SUMMARY : V_TEST_DATA_REPORT_SUMMARYDTO
    {
        ...
        /// <returns></returns>
        public List<KeyValuePair<int, Dictionary<int, List<V_TEST_DATA_REPORT_SUMMARYDTO>>>> getTestStpReportDataByTestStoryBoardIdWithDataSrc(string strDBIdx,
            long testStoryBoardId,
            ref string strError, int testMode,
            bool isNormalizationReq = false)
        {
            Logger.Info("getTestStpReportDataByTestStoryBoardIdWithDataSrc", string.Format("story boardId :[{0}]", testStoryBoardId));
            MarsEntities objEntities = BoHelper.GetMarsEntitiesInstance(strCurrentDB: strDBIdx);
            ...
            var query = from vCaptBase in objEntities.V_TEST_DATA_REPORT_SUMMARY
                        ...
                        group vCaptBase by new { vCaptBase.STORYBOARD_DETAIL_ID, vCaptBase.TEST_MODE } into vCaptGroup
                        from vRec in objEntities.V_TEST_DATA_REPORT_SUMMARY
                        from stry_dt in objEntities.T_STORYBOARD_DATASET_SETTING
                        //from stp_data in objEntities.TEST_DATA_SETTING
                        where
                            vRec.STORYBOARD_ID == testStoryBoardId
                            && stry_dt.STORYBOARD_DETAIL_ID == vRec.STORYBOARD_DETAIL_ID  // ??????data set id
                                                                                          //&& stry_dt.DATA_SUMMARY_ID == stp_data.DATA_SUMMARY_ID
                                                                                          //&& stp_data.STEPS_ID == stp_data.STEPS_ID
                            && vRec.LATEST_TEST_MARK_ID == vCaptGroup.Max(p => p.LATEST_TEST_MARK_ID)
                            && vRec.STORYBOARD_DETAIL_ID == vCaptGroup.Key.STORYBOARD_DETAIL_ID
                            && vRec.TEST_MODE == vCaptGroup.Key.TEST_MODE
                            && vRec.TEST_MODE == testMode
                        join stp_data in objEntities.TEST_DATA_SETTING
                            on new { vRec.STEPS_ID, stry_dt.DATA_SUMMARY_ID } equals new { stp_data.STEPS_ID, stp_data.DATA_SUMMARY_ID }
                            into vRecData
                        orderby new { vRec.LOOP_ID, vRec.STORYBOARD_ORDER, vRec.STORYBOARD_DETAIL_ID, vRec.TEST_MODE, vRec.RUN_ORDER }
                        select new { vRec, vRecData };
            ...
            try
            {
                ...
                var objQRaw = query.ToList();
                List<V_TEST_DATA_REPORT_SUMMARY> objQ = new List<V_TEST_DATA_REPORT_SUMMARY>();
                foreach (var objQItm in objQRaw)
                {
                    if (objQItm == null) continue;
                    if (objQItm.vRecData == null)
                    {
                        objQ.Add(objQItm.vRec);
                    }
                    else
                    {
                        if (!string.IsNullOrEmpty(objQItm.vRec.RETURN_VALUES))
                        {
                            //remove the last "\r"
                            if ((!string.IsNullOrEmpty(objQItm.vRec.KEY_WORD_NAME))
                                &&(string.Compare(objQItm.vRec.KEY_WORD_NAME, "CaptureAndCompare", true) == 0))
                            {
                                if (!string.IsNullOrEmpty(objQItm.vRec.RETURN_VALUES))
                                {
                                    if ((!objQItm.vRec.RETURN_VALUES.Equals("\r"))&&(objQItm.vRec.RETURN_VALUES.EndsWith("\r")))
                                    {
                                        objQItm.vRec.RETURN_VALUES = objQItm.vRec.RETURN_VALUES.Substring(0, objQItm.vRec.RETURN_VALUES.Length - 1);
                                    }
                        ...
                        if (objQItm.vRec.INPUT_VALUE_SETTING == null)
                        {
                            if ((objQItm.vRecData == null) || (objQItm.vRecData.FirstOrDefault() == null) || (string.IsNullOrEmpty(objQItm.vRecData.FirstOrDefault().DATA_VALUE)))
                            {
                                objQ.Add(objQItm.vRec);
                                continue;
                            };
                            //?????????
                            objQItm.vRec.INPUT_VALUE_SETTING = objQItm.vRecData.FirstOrDefault().DATA_VALUE;
                        }
                        else
                        {
                            if (System.Text.RegularExpressions.Regex.IsMatch(objQItm.vRec.INPUT_VALUE_SETTING, "_([0-9])+$"))
                            {
                                //???????
                                //??????,
                                objQ.Add(objQItm.vRec);
            ...
            {
                Logger.Error("getTestStpReportDataByTestStoryBoardIdWithDataSrc", string.Format("Exception:[{0}] stackTrace:[{1}]", e.Message, e.StackTrace), e);
                return null;
            }
            finally
            {
                Logger.logEnd("getTestStpReportDataByTestStoryBoardIdWithDataSrc");
            }
        }
            ...
            bool isNormalizationReq = false)
        {
            ...
            try
            {
                ...
                //Dictionary<long?, List<V_TEST_DATA_REPORT_SUMMARY>> orderdQ = objQ.GroupBy(p => p.STORYBOARD_ORDER, p => p).ToDictionary(p => p.Key, p => p.ToList());
                Dictionary<long?, List<V_TEST_DATA_REPORT_SUMMARY>> orderdQ = objQ.GroupBy(p => p.STORYBOARD_DETAIL_ID, p => p).ToDictionary(p => p.Key, p => p.ToList());
        ...
        private string fixLenthOfString(string strSrc, string lastIdx, int targetLen1, ref bool isTheFirst, bool checkNumber = true)
        {
            if (string.IsNullOrEmpty(strSrc)) return strSrc;
            //if (strSrc.Length >= targetLen1) return strSrc;
            int targetLen = targetLen1 - strSrc.Length+1;
            int iPos = strSrc.LastIndexOf(lastIdx, StringComparison.OrdinalIgnoreCase);
            if (iPos < 0) return strSrc;
            
            string tail = strSrc.Substring(iPos + lastIdx.Length);
            string frnt = strSrc.Substring(0, iPos + 1);
            if (checkNumber)
            {
                int tmpInt = 0;
                if (!int.TryParse(tail, out tmpInt)) return strSrc;
                isTheFirst = tmpInt == 1;
                return frnt + tail.PadLeft(targetLen, '0');
            }
            else
            {
                return frnt + tail.PadLeft(targetLen, '0');
        ...
        private Dictionary<int, List<V_TEST_DATA_REPORT_SUMMARYDTO>> NormalizationStepReportData(Dictionary<int, List<V_TEST_DATA_REPORT_SUMMARYDTO>> dicStpInfo)
        {
            ...
            foreach (int iLoop in dicStpInfo.Keys)
            {
                ...
                ///??inputValueSetting??????
                ///
                var lstSrcX = lstSrc.GroupBy(p => p.RUN_ORDER).ToDictionary(p => p.Key, z => z.ToList());
                bool isFirstItm = false;
                List<string> lstOfFirstRowShouldRemove = new List<string>();
                string fixedReturn = "";
                foreach (var ikx in lstSrcX.Keys.OrderBy(p=>p))
                {
                    if (ikx == null) continue;
                    if ((lstSrcX[ikx] == null) || (lstSrcX[ikx].Count <= 1)) continue;
                    int mxLen = lstSrcX[ikx].Max(p => p.INPUT_VALUE_SETTING == null ? -10000 : p.INPUT_VALUE_SETTING.Length);
                    int mnLen = lstSrcX[ikx].Min(p => p.INPUT_VALUE_SETTING == null ? -10000 : p.INPUT_VALUE_SETTING.Length);
                    int iDis = mxLen - mnLen -1;
                    string firstItmTail = iDis <= 0 ?null: "1".PadLeft(iDis, '0');
                    if (iDis != 0)
                    {
                        var itm = lstSrcX[ikx].OrderBy(p => p.INPUT_VALUE_SETTING).ToList();
                        // 第一个是
                        if (itm.Count > 1)
                        {
                            var first = itm[0];
                            fixedReturn = first.INPUT_VALUE_SETTING;
                            bool isFirstRowEmpty = false;
                            if (itm.Count >= 2)
                            {
                                var secnd = itm[1];
                                // AF added additional null checking for "secnd != null"  and secnd.RETURN_VALUES
                                //isFirstRowEmpty = (secnd != null && secnd.INPUT_VALUE_SETTING != null) &&
                                //                  (secnd.RETURN_VALUES != null && secnd.RETURN_VALUES.Equals("\r"));
                                /// replaced by tiger
                                /// 
                                isFirstRowEmpty = (secnd != null && secnd.INPUT_VALUE_SETTING != null) && (
                                                  (secnd.RETURN_VALUES != null && secnd.RETURN_VALUES.Equals("\r"))
                                                  || (secnd.RETURN_VALUES != null && secnd.RETURN_VALUES.Equals("\r\n"))
                                                  || (secnd.RETURN_VALUES != null && secnd.RETURN_VALUES.Equals("\n"))
                                                  );
                            }
                            // 将之后所有的数据重新组合
                            //if (isFirstRowEmpty)
                            //    itm.RemoveAt(1);
                            //fixedReturn = String.Join("\r\n",itm.Select(p=>p.RETURN_VALUES));
                            ////first.RETURN_VALUES ="\r\r\n"+fixedReturn;
                            //first.RETURN_VALUES = "\r\r\n" + fixedReturn;

                            if ((string.IsNullOrEmpty(first.RETURN_VALUES))||(first.RETURN_VALUES.Equals("\r")) || (first.RETURN_VALUES.Equals("\r\n")))
                            {
                                int iskipNum = 1;
                                if (itm.Count >= 2)
                                {
                                    ...
                                    /// replaced by tiger
                                    /// 
                                    if ((secnd != null && secnd.INPUT_VALUE_SETTING != null) && (
                                                      (secnd.RETURN_VALUES != null && secnd.RETURN_VALUES.Equals("\r"))
                                                      || (secnd.RETURN_VALUES != null && secnd.RETURN_VALUES.Equals("\r\n"))
                                                      || (secnd.RETURN_VALUES != null && secnd.RETURN_VALUES.Equals("\n"))
                                                      ))
                                ...
                                fixedReturn = string.Join("\r\n", itm.Skip(iskipNum).Select(p=>p.RETURN_VALUES));
                            }
                            else
                            {
                                fixedReturn = string.Join("\r\n", itm.Select(p => p.RETURN_VALUES));
                            }
                            first.RETURN_VALUES = fixedReturn;
                        }
                    }
                    if (mxLen <= 0) continue;
                    
                    foreach (var itm in lstSrcX[ikx])
                    {
                        if (itm == null) continue;
                        if (string.IsNullOrEmpty(itm.INPUT_VALUE_SETTING)) continue;
                        isFirstItm = false;
                        itm.INPUT_VALUE_SETTING = fixLenthOfString(itm.INPUT_VALUE_SETTING, "_", mxLen,ref isFirstItm);
                        if ((itm.INPUT_VALUE_SETTING.Length== mnLen) && (mnLen != mxLen))
                        {

                        }
                        //if (isFirstItm)
                        //{
                        //    if (!(string.IsNullOrEmpty(itm.RETURN_VALUES) || itm.RETURN_VALUES == "\r")) lstOfFirstRowShouldRemove.Add(itm.INPUT_VALUE_SETTING);
                        //}
                    }
                }
                //lstSrc = lstSrc.OrderBy(p => p.RUN_ORDER).OrderBy(p => p.INPUT_VALUE_SETTING).ToList();
                lstSrc = lstSrc.OrderBy(p => p.RUN_ORDER).ThenBy(p => p.INPUT_VALUE_SETTING).ToList();
                #endregion
                ...
                for (int i = 0; i < lstSrc.Count; i++)
                {
                    ...
                    {
                        //if (lstOfFirstRowShouldRemove.IndexOf(tmpStpInfo.INPUT_VALUE_SETTING) >= 0) continue;
