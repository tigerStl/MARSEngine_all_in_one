    ...
    public class B_TEST_REPORT_STEPS : T_TEST_REPORT_STEPSDTO
    {
...
#if !_forWebSvc
        public static bool InsertTestStepResultForKeyCompare(long? lRptId, long? stpId, DateTime? beginTime,
...
#endif
        {
            ...
            try
            {
                ...
                //string strRunningError,
                //ref string strError)
                bool isOk = clnt.InsertTestStepResultForKeyCompare(lRptId, stpId, beginTime, iSuccessId, lstObjectNameAndValues,
...
#endif
        {
            ...
            try
            {
                ...
                if (objDBObj != null)
                {
                    ...
                    {
                        if (updateObj.RETURN_VALUES.Length > 64)
                        {
                            Logger.Warnning("UpdateAndInsertList", string.Format("Data to save is longer than 64,which is [{0}], only the first 63 is kept", updateObj.RETURN_VALUES.Length));
                            updateObj.RETURN_VALUES = updateObj.RETURN_VALUES.Substring(1, 63);
                        }
                    }
