...
namespace Mars.autoTest.report.Word
{
    ...
    class TestStepsReportGridData 
    {
        ...
        private string getStringWithRowsAndSize(string strSrc, int iRowCnt, int iSize)
        {
            ...
            #region new splite
            string[] arrNoReturns = strSrc.Split(new string[] { "\r\n","\n"  }, StringSplitOptions.RemoveEmptyEntries);
            #endregion

            string strRslt = "";
            while (bCnt)
            {
                #region fix string ends with "\r"
                if (!string.IsNullOrEmpty(arrNoReturns[icurrentRowCnt++]) && (arrNoReturns[icurrentRowCnt++].Length>1)
                    && (arrNoReturns[icurrentRowCnt++].EndsWith("\r")))
                {
                    arrNoReturns[icurrentRowCnt++].Remove(arrNoReturns[icurrentRowCnt++].Length - 1);// = arrNoReturns[icurrentRowCnt++].Substring(0)
                }
                #endregion
    ...
    class TestStoryBoardSummayGridData 
    {
        ...
        /// tiger:20210402 merge into multiple DB mode
        /// </summary>
        /// <returns>false, no data; true: has row</returns>
        //public bool BeginFetchRows()
        public bool BeginFetchRows(string dbIdx)
        {
            ...
            currentStoryBoardInfo = B_V_STORYBOARD_TEST_FULLVISION.GetStoryBoards(this.currentStoryBoardId, dbIdx);
            /// sorted
