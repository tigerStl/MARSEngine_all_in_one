    ...
    public class Patch
    {
        ...
        public override string ToString()
        {
            ...
            foreach (Diff aDiff in this.diffs)
            {
                ...
                // AF    text.Append(HttpUtility.UrlEncode(aDiff.text,
                // AF        new UTF8Encoding()).Replace('+', ' ')).Append("\n");
            }
    ...
    public class diff_match_patch
    {
        ...
        public string diff_toDelta(List<Diff> diffs)
        {
            ...
            foreach (Diff aDiff in diffs)
            {
                ...
                {
                    case Operation.INSERT:
                        // AF text.Append("+").Append(HttpUtility.UrlEncode(aDiff.text,
                        // AF new UTF8Encoding()).Replace('+', ' ')).Append("\t");
                        break;
                    case Operation.DELETE:
            ...
            foreach (string token in tokens)
            {
                ...
                switch (token[0])
                {
                    case '+':
                        ...
                        // AF  param = HttpUtility.UrlDecode(param, new UTF8Encoding(false, true));
        ...
        public List<Patch> patch_fromText(string textline)
        {
            ...
            while (textPointer < text.Length)
            {
                ...
                while (textPointer < text.Length)
                {
                    ...
                    // AFline = HttpUtility.UrlDecode(line, new UTF8Encoding(false, true));
                    if (sign == '-')
                    {
                        // Deletion.
