...
namespace Route2NSEx.src.Marquis.systemUtil
{
    public class MLogger
    {
        ...
        public log4net.ILog mobjLog = log4net.LogManager.GetLogger(LOGGER_NAME);
        ...
        public static MLogger GetLogger(string className = "MarsLog")
        {
            if (!IsLoad)
            {
                setConfigFile();
        ...
        public void logEnd(string strMethodName, string strInfo)
        {
            mobjLog.Info(string.Format("[INFO] {1}.{2} end\t{3}", DateTime.Now, mstrCurrentClassName, strMethodName, strInfo));
        }
