...
namespace com.Mars.MarsTestingFrame
{
    public class MarsTestFrameMain
    {
         ...
         * */
        private ERROR_CODE RunTestBatchFile(FrameWorkStartMode startMode = FrameWorkStartMode.FWSM_Normal,string strProjectID="45", string strApplicationID="1",bool isBaseLineTest=false)
        {
            ...
            if ((startMode != FrameWorkStartMode.FWSM_AUTOGEN_SCRIPTS)&&(startMode!=FrameWorkStartMode.FWSM_STEPMODE))
            {
                ...
                */
                ...
                //this.mobjFrameworkService.NewTestStoryBoardBegins(strProjectID, isBaseLineTest);
...
//#endif
        ...
        public string GetVariableValue(string variable)
        {
            ...
            if ((variableLocalOrGlobal.Equals(SystemConstant.CNST_RESERVED_VARIABLE_GLOBAL))
                ...
                tempValue = BoHelper.GetVariableValue("TEMPSTUB", variableLocalOrGlobal, variableName);
            else
            {
                ...
                else
                {
                    if (string.Compare(SystemConstant.CNST_RESERVED_VARIABLE_IFVAR, variableLocalOrGlobal, true) == 0)
                    {
                        short sStatus = 1;
                        string strError = "";
                        if (!BoHelper.GetIFVariableInfo("TEMPSTUB", variableName, sStatus, ref strError, ref tempValue))
                            tempValue = "";
                    }
                    else
                        tempValue = "";
