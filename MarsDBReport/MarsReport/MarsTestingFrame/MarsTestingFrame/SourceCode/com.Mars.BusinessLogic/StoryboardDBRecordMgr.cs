...
namespace MarsTestFrame.SourceCode.com.Mars.BusinessLogic
{
    ...
    internal class StoryboardDBRecordMgr
    {
        ...
        internal void Initialization(string strStoryBoardId, bool isBaseLine,bool isContinue2Test)
        {
        ...
        internal void CreateNewTestMarkGroupId()
        {
            if (!_isContinueToTest)
                _currentTestMarkGroupId = BoHelper.GetLastestTestMarkID();
            else
            {
                long l_currentStoryBoardId = -1;
                if (long.TryParse(this._currentTestStoryboardId, out l_currentStoryBoardId))
                {
                    _currentTestMarkGroupId = BoHelper.GetLastestTestMarkIDByStoryBoardId(l_currentStoryBoardId);
                }
                else
                {
        ...
        internal void UpdateTestReportResult(int idSuccess, string strErrorInfo)
        {
            ...
            try
            {
                ...
                int iError = BoHelper.UpdateStoryBoardTestResult(storyBoardTestResult, ref strDBError, "TEMPSTUB");
                if (iError != 1)
                {
        ...
        internal int CreateStepLog(long stepId, TestStepRunningRecorder objCurrentStepLog)
        {
            ...
            try
            {
                ...
                int  iError = objStepRpt.CreateIdAndSave(ref strErrror,ref strAdv, "TEMPSTUB");
                if (iError<0)
                {
        ...
        internal int UpdateCurrentTestStepResult(int iLoopId,long assignedTestStepId, int idSuccess, string strError,byte[] arrPicInfo)
        {
            ...
            try
            {
                return objTestStepLog.updateRecord("TEMPSTUB");
            }
            catch (Exception e)
            {
        ...
        internal int StoreData4ForTestReport_Steps(string strObjectNameIdx, string strData2Store, int iLoopId, long? testCaseKeyId, long assignedTestStepId)
        {
            ...
            int iResult = B_TEST_REPORT_STEPS.UpdateAndInsertList(objTestStepLog,lstTestStepReport, strCurrentObjName, "TEMPSTUB");
            return iResult;
        }
        ...
        internal int LogTestCaseStart(long? testCaseKeyId, int iLoop)
        {
            ...
            try
            {
                ...
                int iErrorId = objTestCaseLog.Create2Entities(ref strError, "TEMPSTUB");
                if (iErrorId<0)
                {
        ...
        internal int UpdateTestCaseLoopResult(long? testCaseKeyId, int iResultId, int iLoopId, string strEndInfo)
        {
            ...
            try
            {
                ...
                int iError = objCurrentLoopRecord.updateById(ref strError, "TEMPSTUB");
                if (iError<0)
                {
        ...
        internal int UpdateTestCaseLoopId(int iPos, int iLoopId)
        {
            ...
            try
            {
                if (currentTestCaseLog.TryGetValue(iPos, out objTarget))
                {
                    ...
                    int iError= objTarget.update("TEMPSTUB");
                    if (iError < 0)
                    {
        ...
        internal bool updateVariableValue(string strObjectNameIdx, string strData2Store, int iLoop,int iVariableType,ref string strError,string strBaseLineMode= "Build")
        {
                ...
                switch (iVariableType)
                {
                    ...
                    case 8://IF Var
                        ...
                        if (iscreateNew)
                        {
                            ...
                            isRight = objLook.InsertSelf("TEMPSTUB",ref strError);
                        }
                        else
                        {
                            lstObj = ((lstObj.OrderBy(p => p.DISPLAY_NAME)).ToList());
                            isRight = lstObj[0].updateSelf(strData2Store, ref strError, "TEMPSTUB");
                        }
                        return isRight;
                    case 4://modal var
                        int iStatus = string.Compare(strBaseLineMode,SystemConstant.CNST_APPCONFIG_APPSETTING_BASELINEMODE_BUILD, true) == 0 ? 1 : 2;                        
                        lstObj = objLook.GetSystemLookup(SystemConstant.CNST_RESERVED_VARIABLE_MODAL, strObjectNameIdx, iStatus, ref strError, "TEMPSTUB");
                        iscreateNew = lstObj == null ? true : lstObj.Count > 0 ? false : true;
                        if (iscreateNew)
                        {
                            objLook.DISPLAY_NAME = strData2Store;
