...
namespace Mars.Business
{
    public class B_SYSTEM_LOOKUP : SYSTEM_LOOKUPDTO, INotifyPropertyChanged
    {
     ...
     public ObservableCollection<KeyValuePair<string, int>> BaseOrCompareDic
        ...
        public bool InsertSelfWithStatus(ref string strError, MarsEntities objDBCntx = null,string strDBIdx ="MarsEntities")
        {
            ...
            try
            {
                ...
                if (oE != null)
                {
                    ...
                    Logger.Error("InsertSelfWithStatus", strError = string.Format("can't find object from system_lookup with info:[{0}]", strError));
                    return false;
                }
        ...
        {
            Logger.Info("createModualvar", string.Format("varibals:[{0}]", lstName==null?"":string.Join(",", lstName)));           
            try
            {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (oE != null)
                    {
                        ...
                        //Logger.Error("updateSelf", strError = string.Format("can't find object from system_lookup with id:[{0}]", this.LOOKUP_ID));
                        return false;
                    }
                }
