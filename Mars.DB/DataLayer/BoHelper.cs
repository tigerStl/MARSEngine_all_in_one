...
using Mars.message.Utility.ToleranceMgr;
            ...
            string strCurrentDB = MarsEntitiesExtends.cnst_default_dbName)
        {
            ...
            try
            {
                ...
                if (gMarsEntites == null)
                {
                    ...
                    gMarsEntites = new Mars.Model.MarsEntities(MarsEntitiesExtends.CachedConnectionStrings[strCurrentDB].GetEntityConnection()); 
...
#region B_TEST_STEPS realted access methods
        ...
        public static List<T_TEST_STEPSDTO> GetTestStepByName(string strDBIdx, string strTestCaseName, MarsEntities marsdbCntx, ref string strError, ref bool isOk)
        {
            ...
            MarsEntities dbCntx = marsdbCntx == null ? Mars.message.DataLayer.BoHelper.GetMarsEntitiesInstance(strCurrentDB: strDBIdx) : marsdbCntx;
            try
            {
...
#region for testing
            ...
            string strDBIdx = MarsEntitiesExtends.cnst_default_dbName)
        {
            ...
            try
            {
                ...
                //List<B_SYSTEM_LOOKUP> lstObjs = lstModalVars;
                if ((lstObjs == null) || (lstObjs.Count == 0))
                {
                ...
                // for debug
                lstObjs.ForEach(p =>
                    Logger.Info("GetModalVariableInfo", $"variable is : [{p.DISPLAY_NAME},value:{p.VALUE}]-status:[{p.STATUS}]")
                );
                ...
                strResult = q.DISPLAY_NAME;
                Logger.Info("GetModalVariableInfo", $"{strModalVarIdx}-[{strResult}]");
                return true;
            }
            catch (Exception e)
            {
        ...
        public static bool GetLoopVaribleInfo(string strLoopVarIdx, ref string strError, ref string strResult,string strDBIdx)
        {
            ...
            {
                Logger.Info("GetLoopVaribleInfo", $"loop varis return:[{strResult}]");
...
#if !_forWebClient
        public static string GetDynamicDataByStoryBoardInfoAndObjectName(string strDBIdx, long l_storyBoardDetailId, long? lDataSettingId, 
            string strObjectName, int iBaseLineId, int iLoop,ref bool isRight)
        {
...
#region clean duplicated object records
        ...
        internal static void UpdateDataSource(string strDBIdx, string id, string data, short dataType,MarsEntities dbCntx=null)
        {
            MarsEntities marsEntities = BoHelper.GetMarsEntitiesInstance(strCurrentDB: strDBIdx);
        ...
        internal static void DeleteDataSource(string strDBIdx, string id, short dataType)
        {
            MarsEntities marsEntities = BoHelper.GetMarsEntitiesInstance(strCurrentDB: strDBIdx);
            var dataSource = (from o in marsEntities.T_DATA_SOURCE
                              where o.DATA_SOURCE_NAME == id
