            ...
            long? currentStoryBoardDetailId)
        {
            ...
            try
            {
                var query = from vCaptBase in objEntities.V_TEST_DATA_REPORT_SUMMARY
                            ...
                            select vRec;
                Logger.Info("GetTestReportByStoryBoardDetailId", $"sql Gen:\r\n{query.ToString()}");
                Logger.Info("GetTestReportByStoryBoardDetailId", string.Format("Datareturnd:[{0}]", query.ToList().Count));

                var queryM = from vCaptBase in objEntities.V_TEST_DATA_REPORT_SUMMARY
                             where vCaptBase.STORYBOARD_DETAIL_ID == currentStoryBoardDetailId
                             group vCaptBase by new { vCaptBase.STORYBOARD_DETAIL_ID, vCaptBase.TEST_MODE } into vCaptGroup
                             select new { dtlAndMod = vCaptGroup.Key, mxLatest = vCaptGroup.Max(s => s.LATEST_TEST_MARK_ID) };
                Logger.Info("GetTestReportByStoryBoardDetailId", $"sql gen:\r\n{queryM.ToString()}");
                var queryX = from vRec in objEntities.V_TEST_DATA_REPORT_SUMMARY
                             where
                                vRec.STORYBOARD_DETAIL_ID == currentStoryBoardDetailId
                             select vRec;
                Logger.Info("GetTestReportByStoryBoardDetailId", $"sql queryX gen:\r\n{queryX.ToString()}");
                var x = from z in queryX
                        from m in queryM
                        where z.STORYBOARD_DETAIL_ID == m.dtlAndMod.STORYBOARD_DETAIL_ID
                        && z.TEST_MODE == m.dtlAndMod.TEST_MODE
                        && z.LATEST_TEST_MARK_ID == m.mxLatest
                        select z;
                Logger.Info("GetTestReportByStoryBoardDetailId", $"get x :{x.ToString()}");
                Logger.Info("GetTestReportByStoryBoardDetailId", $"get x count:{x.ToList().Count}");
            ...
            {
                Logger.logEnd("GetTestReportByStoryBoardDetailId");
            }
        }
...
#region tiger
            var query = from vCaptBase in objEntities.V_TEST_DATA_REPORT_SUMMARY                        
                        ...
                        where
                            ...
                            && stry_dt.STORYBOARD_DETAIL_ID == vRec.STORYBOARD_DETAIL_ID  // 定位到指定的data set id
            ...
            try
            {
                ...
                Logger.Info("\t", $"getTestStpReportDataByTestStoryBoardIdWithDataSrc:{query.ToString()}");
...
#region tiger
        ...
        private string fixLenthOfString(string strSrc, string lastIdx, int targetLen1, bool checkNumber = true)
        {
...
#region tiger changed
                ...
                foreach(var ikx in lstSrcX.Keys)
                {
                    ...
                    foreach(var itm in lstSrcX[ikx])
                    {
                        ...
                        itm.INPUT_VALUE_SETTING = fixLenthOfString(itm.INPUT_VALUE_SETTING, "_", mxLen);
                    }
                }
