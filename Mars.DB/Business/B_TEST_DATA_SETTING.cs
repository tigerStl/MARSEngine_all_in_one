...
namespace Mars.message.Business
{
    public class B_TEST_DATA_SETTING : TEST_DATA_SETTINGDTO
    {
        ...
        public Dictionary<long, List<TEST_DATA_SETTINGDTO>> GetTestDataByStoryboardID(long storyboardId, string strDBIdx, ref string strError)
        {
            Logger.logBegin("GetTestDataByStoryboardID", $"storyboard id:[{storyboardId}]");
            MarsEntities objEntities = BoHelper.GetMarsEntitiesInstance(true, strDBIdx);
            bool isOk = false;
            try
            {
                var query = from stry in objEntities.T_PROJ_TC_MGR
                                //from tc in objEntities.T_TEST_CASE_SUMMARY 
                            from stp in objEntities.T_TEST_STEPS
                            from s_d in objEntities.T_STORYBOARD_DATASET_SETTING
                            from dt in objEntities.TEST_DATA_SETTING
                            where stry.STORYBOARD_ID == storyboardId
                            //&& tc.TEST_CASE_ID == stry.TEST_CASE_ID
                            && stp.TEST_CASE_ID == stry.TEST_CASE_ID
                            && s_d.STORYBOARD_DETAIL_ID == stry.STORYBOARD_DETAIL_ID
                            && dt.DATA_SUMMARY_ID == s_d.DATA_SUMMARY_ID
                            && stp.STEPS_ID == dt.STEPS_ID                            
                            select new { dt, stry.STORYBOARD_DETAIL_ID };
                var d = query.GroupBy(p => p.STORYBOARD_DETAIL_ID, v => v.dt).ToDictionary(p => p.Key, z => z.ToList().ToDTOs());
                isOk = true;
                strError = "";
                return d;                
            }
            ...
            {
                Logger.Error("GetTestDataByStoryboardID", strError = e.Message, e);
                isOk = false;
                return null;
            }
            finally
            {
                Logger.logEnd("GetTestDataByStoryboardID", $"execute [{isOk}]");
            }
        }
